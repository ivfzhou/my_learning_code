...
- **eval(*codeStr*)**：评估字符串并像脚本代码一样执行它。
- **postMessage()**：向 Worker 发送消息。
- **fetch(*url*)**：返回 Promise 对象。结合 await 使用返回 Response。
- **encodeURI(*str*)**：对 URI 进行编码。不对以下字符编码：, / ? : @ & = + $ #。
- **decodeURI(*str*)**：解码 URI。不对以下字符编码：, / ? : @ & = + $ #。
- **encodeURIComponent(*str*)**：对 URI 组件进行编码。
- **decodeURIComponent(*str*)**：解码 URI 组件。
- **import(*pathJs*)**：动态导入 js 文件，返回 Promise 对象。
