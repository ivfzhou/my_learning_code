...
2. 字段序号范围[1, 2^<sup>29</sup>-1(536870911)]，[19000, 19999]为字段描述占用。
3. 语言数据类型是 32 位，但电报数据是 64 位，则会把多余位舍弃。
4. sint32 最大值 2^<sup>31</sup>-1，最小值 -2^<sup>31</sup>。sint64 最大值 2^<sup>63</sup>-1，最小值 -2^<sup>63</sup>。
...
8. sint/zigzag 编解码，编码：32 位：n<<1 ^ n>>31、64 位：n<<1 ^ n>>63，解码：if x % 2 == 0; (x ^ 0) >> 1 else (x ^ -1) >> 1。
