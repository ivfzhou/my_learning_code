...
- sed：文本查找替换处理加工。如果斜线匹配冲突可以换成别的符号作分割符。
    - 's/old/new/' *file* 替换每行第一个匹配的字符串，文本结果打印到输出，不修改原文件。
    - 's/old/new/g/' *file* 替换每行匹配的所有字符串，文本结果打印到输出，不修改原文件。
    - '2s/old/new/' *file* 替换第二行匹配的第一个字符串，文本结果打印到输出，不修改原文件。
    - '2,$s/old/new/' *file* 只匹配第二行到最后一行。
    - '3d' *file* 删除第三行，文本结果打印到输出，不修改原文件。
    - '/regular/d' *file* 删除匹配的行。
    - '2,5d' *file* 删除第二到第五行，文本结果打印到输出，不修改原文件。
    - '/^$/d' *file* 删除空行，文本结果打印到输出，不修改原文件。
    - '/hit/d' *file* 删除包含字符串的行，文本结果打印到输出，不修改原文件。
    - '/hit/d' -i *file* 删除包含字符串的行，修改了原文件，不打印输出。
    - -e '/^#/d' -e '/^$/d' *file* 删除注释行和空行，结果打印到输出。
    - '2i\text' *file* 在第二行前插入新行字符串，输出结果。
    - '/regular/i text' *file* 将匹配行向上插一行字符串。
    - '2a\text' *file* 在第二行后插入新行字符串，输出结果。
    - '/regular/a text' *file* 将匹配行向下插一行字符串。
    - '3p' -n *file* 打印文本第三行。
    - '3,5p' -n *file* 打印文本第三行到第五行。
    - '/regular/p' *file* 将匹配行输出。
    - 's/old/new;s/old/new/;s!old!new!' *file* 在一个脚本字符串中串联多条命令。
    - 's/支持扩展正则表达式/new/' -r *file*
    - 's/(text)abc/\1/g' -r *file* 删除紧跟在 text 后面的 abc。其中 `\1` 表示回调一组字符串。
    - 's/old/new/2' -r *file* 每行匹配替换两次。
    - '/regular/{s///;s///}' *file* 大括号中命令分组，多个命令作用于同一个匹配行。
    - '/regular/=' *file* 匹配行，然后打印行号。
    - '/regular/c text' *file* 将匹配行改成字符串 text。
    - '/regular/r file' *file* 将匹配行向下插入指定文件的内容。
    - '/regular/w file' *file* 将匹配的行写入文件中。
    - '/regular/n' *file* 读取下一行替换模式空间。
    - '/regular/,10q' *file* q 是退出命令。
    - 'N;s//\n/;P;D' *file* N 将下一行加入到模式空间。D 删除模式空间第一个字符到第一个换行符。P 打印模式空间第一个字符到第一个换行符。
    - '1h;1!G;$!x;$p' *file*
    - h：将模式空间内容复制到保持空间（覆盖原内容）。
    - H：将模式空间内容追加到保持空间（换行后再追加）。
    - g：将保持空间内容复制到模式空间（覆盖当前行）。
    - G：将保持空间内容追加到模式空间（换行后再追加）。
