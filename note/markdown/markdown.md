# 一、笔记

1. [CommonMark](https://commonmark.org/) 标准文档。
1. [GitHub Flavored Markdown (GFM)](https://github.github.com/gfm/) 标准文档。
1. GFM 是 CommonMark 的严格超集。CommonMark 不支持表格、任务列表和删除线。

# 二、语法

## 2.1 标题

行首井号，后跟一个空格：`# 标题`。

### 2.1.1 链接到标题

在标题后面使用花括号，花括号中写 ID，形如：`# title {#id}`，链接中可指定标题：`[title](#id)`。

## 2.2 段落

一个或多个空白行。

## 2.3 换行

行尾两个空格。

## 2.4 强调

### 2.4.1 加粗

用两个星号包裹：`**文本**`。

### 2.4.2 斜体

用一个星号包裹：`*文本*`。

### 2.4.3 粗斜体

用三个星号包裹：`***文本***`。

### 2.4.5 删除线

用两个波浪线包裹：`~~文本~~`。

## 2.5 引用

行首右尖括号，后跟一个空格：`> 文本`。

### 2.5.1 块引用

每行行首右尖括号。在其中可以结合使用代码块和列表。

```markdown
> 文本
>
> 文本
```

### 2.5.2 嵌套块引用

块引用行首再加一个右尖括号。

```markdown
> 文本
>
> > 文本
```

## 2.6 列表

### 2.6.1 有序列表

行首数字 1，后跟小数点和空格：`1. 文本`。

### 2.6.2 无序列表

行首中划线，后跟一个空格：`- 文本`。

### 2.6.3 列表中可使用其他语法

使用四个空格或一个制表符缩进，代码块需要八个空格或两制表符缩进。

## 2.7 代码

### 2.7.1 小段代码

文本前后一个或多个反引号：`` `文本` ``。

### 2.7.2 代码块

- 行首缩进四个空格或者一个制表符。
- 使用三个反引号包围也可以形成代码块，并可在前反引号后指定一种语言。
  ````markdown
  ```language
  文本
  ```
  ````

## 2.8 Mermaid 图

### 2.8.1 流程图

````markdown
```mermaid
graph TD;
    A[开始] --> B{判断};
    B -->|是| C[执行A];
    B -->|否| D[执行B];
    C --> E[结束];
    D --> E;
```
````

```mermaid
graph TD;
    A[开始] --> B{判断};
    B -->|是| C[执行A];
    B -->|否| D[执行B];
    C --> E[结束];
    D --> E;
```

### 2.8.2 序列图

````markdown
```mermaid
sequenceDiagram;
    Alice->>John: 你好 John, 你好吗？
    John-->>Alice: 我很好，谢谢！
```
````

```mermaid
sequenceDiagram;
    Alice->>John: 你好 John, 你好吗？
    John-->>Alice: 我很好，谢谢！
```

### 2.8.3 甘特图

````markdown
```mermaid
gantt
    title 项目进度
    dateFormat YYYY-MM-DD
    section 设计
    设计阶段 :a1, 2023-01-01, 30d
    section 开发
    开发阶段 :after a1, 60d
```
````

```mermaid
gantt
    title 项目进度
    dateFormat YYYY-MM-DD
    section 设计
    设计阶段 :a1, 2023-01-01, 30d
    section 开发
    开发阶段 :after a1, 60d
```

### 2.8.4 类图

````markdown
```mermaid
classDiagram
    Class01 <|-- AveryLongClass : 继承
    Class03 *-- Class04 : 组合
    Class05 o-- Class06 : 聚合
```
````

```mermaid
classDiagram
    Class01 <|-- AveryLongClass : Cool
    Class03 *-- Class04
    Class05 o-- Class06
```

### 2.8.5 状态图

````markdown
```mermaid
stateDiagram-v2
    [*] --> 状态1
    状态1 --> 状态2
    状态2 --> [*]
```
````

```mermaid
stateDiagram-v2
    [*] --> 状态1
    状态1 --> 状态2
    状态2 --> [*]
```

### 2.8.6 饼图

````markdown
```mermaid
pie
    title 饼图示例
    "部分1": 30
    "部分2": 70
```
````

```mermaid
pie
    title 饼图示例
    "部分1": 30
    "部分2": 70
```

## 2.9 分割线

一行三个下划线：`___`。

## 2.10 链接

语法：`[超链接显示名](超链接地址 "超链接 title")`。可将网址和电子邮箱号使用尖括号包围，变成可点击链接：`<ivfzhou.com>`。

### 2.10.1 引用型链接

由两部分组成：
- 第一部分有两个方括号，第一个方括号包围应显示为链接的文本，第二个方括号指向链接：`[超链接显示名][1]`。
- 第二部分由方括号，后跟冒号和空格，再后面是链接和双引号括起来的标题：`[1]: http://ivfzhou.cn "title"`。

### 2.10.2 图片

语法：`![图片 alt](图片链接 "图片 title")`。图片形式的链接：`[![图片 alt](图片链接 "图片 title")](链接)`。

## 2.11 表格

三个连字符生成列，一个管道符生成行，使用冒号表示对齐方式。表格中的 | 符号需要转义。
```markdown
| 列名 | 列名 |
| :---: | :---: |
| 数据 | 数据 |
```

## 2.12 HTML

文本中编写 HTML 代码将被解释。

## 2.13 任务列表

语法：`- [ ]`，已完成的语法：`- [x]`。

## 2.14 LateX

### 2.14.1 行内公式

用 $ 包围：`$\frac{1}{2}$`。

### 2.14.2 块级公式

用两个 $$ 包围：
```markdown
$$
\frac{1}{2}
$$
```

或者，用 \\[ 和 \\] 包围：
```latex
\[
\frac{1}{2}
\]
```

# 三、YAML Frontmatter（元数据配置块）

YAML Frontmatter 不是 CommonMark 或 GFM 的标准语法，且它必须位于文件的最顶部才生效。  
语法：
```markdown
---
conent
---
```
