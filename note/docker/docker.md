...
1. 删除数据卷文件可能导致 run 失败，运行 systemctl restart docker 重启修复。
...
1. 获取 docker-compose：
   ```bash
   mkdir ~/bin
   curl -L -o ~/bin/docker-compose https://github.com/docker/compose/releases/download/v5.0.2/docker-compose-linux-x86_64
...
1. 运行 git clone https://github.com/moby/moby.git。
1. 运行 cd moby。
1. 从 https://download.docker.com/linux/static/stable/ 下载编译好的 docker 程序包。
1. 解压 docker 程序包，将文件放到 docker_boot/ 下。
1. 从 https://github.com/docker/buildx/releases 下载 docker-buildx。
1. 将 docker-buildx 可执行程序放到 ~/.config/cli-plugins/ 下，将权限设置为可执行。
1. 将 docker_boot 文件夹设置到环境变量 PATH 中。
1. 配置 docker 网络代理，生成文件 config.json。
1. root 用户运行 dockerd --userland-proxy=false --config-file=config.json --bip=172.17.0.1/16。
1. 配置 Docker 容器内的网络代理，在 Dockerfile 中，分别在运行 apt 和 go 命令处添加：
   ```dockerfile
   ...
   ```
1. 运行 make。
1. 生成可执行文件在 bundle 文件夹下。
1. 运行 sudo ip link delete docker0 删除网络接口。

## 2.3 二进制包方式

1. 下载二进制压缩包，https://download.docker.com/linux/static/stable。
1. 解压，设置环境变量。
1. 运行 `sudo env PATH=$PATH:/sbin dockerd`。
...
1. docker exec *参数* *containerName* *args*：进去运行中的容器。args 表示进入容器后执行的命令，如果需要和容器进行交互式访问，需要配置参数为 -it，并且命令设置为 bash（表示通过命令行的方式交互访问容器）。
     - -i：保持和 docker 容器内的交互。
     - -t：为容器的标准输入，虚拟一个 tty。
     - -w：指定进入容器后的工作路径。
...
1. 配置文件增加字段 "insecure-registries":[ *host* ]，表示 push 时不使用校验 SSL 证书。
1. 配置 Docker 网络代理：
   ...
   1. 运行 sudo systemctl restart docker。
