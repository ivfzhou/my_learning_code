...
public class AfterSaleTools {
    ...
    @Tool(
            name = "query_order",
            readOnly = true,
            description = "查询订单快照：商品、实付金额、优惠构成、下单/发货/签收时间、订单状态。需要确认“发生了什么”时先调它。"
    )
    public String queryOrder(@ToolParam(name = "order_id", description = "订单号") String orderId) {
        return """
                订单号：%s
    ...
    public String queryLogistics(@ToolParam(name = "order_id", description = "订单号") String orderId) {
        return """
                运单号：SF%08d
                承运商：顺丰速运
                轨迹：
    ...
    public String calcRefund(
            @ToolParam(name = "order_id", description = "订单号")
            String orderId,
    ...
    public String queryUserRiskProfile(@ToolParam(name = "user_id", description = "用户 ID") String userId) {
        return """
                用户 ID：%s
                近 90 天订单数：14
                近 90 天退货数：3（退货率 21%%）
    ...
    public String createAftersaleTicket(
            ...
            String type,
            @ToolParam(name = "amount", description = "涉及金额，单位元")
            double amount,
    ...
    public String draftReply(
            @ToolParam(name = "conclusion", description = "结论：退款/换货/驳回/需人工复核")
            String conclusion,
            ...
            String amount,
            @ToolParam(name = "deadline", description = "承诺时效，必须是具体日期")
            String deadline,
