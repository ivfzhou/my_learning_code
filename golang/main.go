...
func main() {
	...
	case 16:
		file, err := os.ReadFile(`/home/ivfzhou/下载/zhaoliu.png`)
		if err != nil {
			panic(err)
		}
		sum := md5.Sum(file)
	...
	case 17:
		println(randomPrintableAsciiString(8))
	case 18:
		const directory = `/home/ivfzhou/文档/Camera/`
		const trash = `/home/ivfzhou/文档/test/`

		// 读取文件夹下所有文件。
		log.Println("INFO", "read directory", directory)
		...
		fileInfos := make(map[string]*FileInfo, len(entries))
		for _, entry := range entries {
			// 跳过文件夹。
			if entry.IsDir() {
				log.Println("WARN", "skip folder", entry.Name())
				continue
			}

			// 计算文件的 MD5 和 SHA1。
			log.Println("INFO", "calculate file md5 and sha1", entry.Name())
			fileInfo, err := entry.Info()
			if err != nil {
				panic(err)
			}
			fileObj, err := os.OpenFile(filepath.Join(directory, entry.Name()), os.O_RDONLY, 0)
			if err != nil {
				panic(err)
			}
			md5Hash := md5.New()
			sha1Hash := sha1.New()
			written, err := io.Copy(io.MultiWriter(md5Hash, sha1Hash), fileObj)
			if err != nil {
				panic(err)
			}
			_ = fileObj.Close()
			if written != fileInfo.Size() {
				log.Println("ERROR", "file size mismatch", fileInfo.Size(), written)
			}
			md5Sum := md5Hash.Sum(nil)
			sha1Sum := sha1.Sum(nil)
			fileInfos[entry.Name()] = &FileInfo{
		...
		// 检查出重复的文件。
		log.Println("INFO", "check duplicate files")
		for _, fileName := range slices.Collect(maps.Keys(fileInfos)) {
			fileInfo, ok := fileInfos[fileName]
			if !ok {
				continue
			}
			log.Println("INFO", "check file", fileName)
			for k, v := range fileInfos {
				...
				// 如果文件信息都相同，就认定是相同文件。
				if fileInfo.MD5 == v.MD5 && fileInfo.SHA1 == v.SHA1 && fileInfo.Ext == v.Ext &&
					fileInfo.Size == v.Size {
					log.Println("INFO", "file is duplicated", k)
					delete(fileInfos, k)
					err = os.Rename(filepath.Join(directory, k), filepath.Join(trash, k))
					if err != nil {
						panic(err)
					}
				}
	...
	case 19:
		file, err := os.ReadFile(`/home/ivfzhou/src/csms/testdata/zhangsan.jpg`)
		if err != nil {
			panic(err)
		}
...
func getMainFuncDirectory() string {
	...
	for {
		...
		if frame.Function == "main.main" {
			directory := filepath.Dir(frame.File)
			index := strings.LastIndex(directory, string(os.PathSeparator))
			if index != -1 {
				directory = directory[index+1:]
			}
			directory = strings.Trim(directory, string(os.PathSeparator))
