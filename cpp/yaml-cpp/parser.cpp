...
namespace gitee::com::ivfzhou::cpp::yaml_cpp {
    ...
    void example_1() {
        ...
        try {
            const auto& object = YAML::LoadFile(get_file_path());
            if (object["lastLogin"]) {
                LOG("lastLogin is", object["lastLogin"].as<std::string>());
            }
        } catch (const YAML::BadFile& e) {
            LOG("yaml 文件未找到：", e.what());
        }
    ...
    void example_2() {
        ...
        try {
            ...
            for (auto v : {"scalarType", "mapType", "sequenceType", "nullType", "undefinedType"}) {
                ...
                case YAML::NodeType::Null: LOG("hit null type", v, std::boolalpha, object[v].IsNull()); break;
                case YAML::NodeType::Scalar: LOG("hit scalar type", v, std::boolalpha, object[v].IsScalar()); break;
                case YAML::NodeType::Sequence:
                    LOG("hit sequence type", v, std::boolalpha, object[v].IsSequence());
                    break;
                case YAML::NodeType::Map: LOG("hit map type", v, std::boolalpha, object[v].IsMap()); break;
                case YAML::NodeType::Undefined:
                    LOG("hit undefined type", v, std::boolalpha, object[v].IsDefined());
                    break;
                default: LOG("unknown node type", type, std::boolalpha, object[v].IsNull()); break;
                }
            }
    ...
    void example_3() {
        ...
        try {
            ...
            if (const auto& sequence = object["sequenceType2"]; sequence && sequence.IsSequence()) {
                for (std::size_t i = 0; i < sequence.size(); i++) std::cout << sequence[i].as<int>() << " ";

                std::cout << std::endl;

                for (auto it = sequence.begin(); it != sequence.end(); ++it) std::cout << it->as<int>() << " ";

                std::cout << std::endl;
            }

            if (const auto& sequence = object["sequenceType3"]; sequence && sequence.IsSequence()) {
                for (const auto& i : sequence) std::cout << i.as<int>() << " ";
    ...
    void example_8() {
        ...
        try {
            ...
            for (const auto v : vector) std::cout << v << " ";
